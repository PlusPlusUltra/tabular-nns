...
def detect_and_log_outliers(split_name, data, dataset_name, method, log_dir="outlier_logs"):
    if method == "IsolationForest":
        ...
        mask = (np.abs(modified_z) < 3.5).all(axis=1)
    ...
    log_path = os.path.join(log_dir, f"{dataset_name}_outliers.txt")
    with open(log_path, "a") as f:
        f.write(f"{dataset_name} - {split_name} ({method}):\n")
        f.write(f"  Outliers detected: {num_outliers}\n")
        f.write(f"  Total samples:     {len(data)}\n\n")

    ############creating samples of outliers and non outliers
    # After computing `mask`:
    outlier_indices = np.where(~mask)[0]
    non_outlier_indices = np.where(mask)[0]

    # Prepare samples
    outlier_samples = data[outlier_indices[:10]]
    non_outlier_samples = data[non_outlier_indices[:10]]

    # Create output directory
    output_dir = os.path.join("outlier_samples")
    os.makedirs(output_dir, exist_ok=True)

    # File name
    fname = f"dataset={dataset_name}__split={split_name}__method={method}.txt"
    fpath = os.path.join(output_dir, fname)

    # Write to file
    with open(fpath, "w") as f:
        f.write("Outliers:\n")
        for i, row in enumerate(outlier_samples):
            f.write(f"#{i+1}: {row.tolist()}\n")

        f.write("\nNon-Outliers:\n")
        for i, row in enumerate(non_outlier_samples):
            f.write(f"#{i+1}: {row.tolist()}\n")
...
def get_dataset(dataset_name, dataset_path, remove_outliers=False, outlier_method="IsolationForest"):
    """
    Load the dataset from the numpy files, optionally apply outlier filtering to training data.
    :param dataset_name: str
    :param dataset_path: str
    :return: Tuple[ArrayDict, ArrayDict, Dict[str, Any]]
    """
    N, C, y, info = dataname_to_numpy(dataset_name, dataset_path)

    N_trainval = None if N is None else {key: N[key] for key in ["train", "val"] if key in N}
    N_test = None if N is None else {key: N[key] for key in ["test"] if key in N}

    C_trainval = None if C is None else {key: C[key] for key in ["train", "val"] if key in C}
    C_test = None if C is None else {key: C[key] for key in ["test"] if key in C}

    y_trainval = {key: y[key] for key in ["train", "val"] if key in y}
    y_test = {key: y[key] for key in ["test"] if key in y}

    if remove_outliers:

        def combine(N_arr, C_arr, method):
            if method in ["ZScore", "ModifiedZScore", "IQR"]:
                return N_arr  # Use only numerical features for statistical methods
            else:
                return np.hstack([N_arr, C_arr]) if C_arr is not None else N_arr


        # Train
        X_train_combined = combine(N_trainval['train'], C_trainval['train'] if C_trainval else None, outlier_method)

        mask_train = detect_and_log_outliers("train", X_train_combined, dataset_name, outlier_method)
        N_trainval['train'] = N_trainval['train'][mask_train]
        if C_trainval:
            C_trainval['train'] = C_trainval['train'][mask_train]
        y_trainval['train'] = y_trainval['train'][mask_train]

        # Val
        X_val_combined = combine(N_trainval['val'], C_trainval['val'] if C_trainval else None, outlier_method)
        _ = detect_and_log_outliers("val", X_val_combined, dataset_name, outlier_method)

        # Test
        X_test_combined = combine(N_test['test'], C_test['test'] if C_test else None, outlier_method)
        _ = detect_and_log_outliers("test", X_test_combined, dataset_name, outlier_method)
